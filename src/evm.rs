...
impl EthereumClient {
  ...
  pub async fn load_remote(&self, address: String, rpc_url: String) -> Result<String, Box<dyn std::error::Error>> {
    let url_str = rpc_url.parse::<Url>()?;
...
mod tests {
  ...
  async fn test_loads_remote_bytecode() {
    ...
    let rpc_url = String::from(client.network.get_default_rpc().unwrap());
